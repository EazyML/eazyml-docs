...
def ez_cf_inference(test_data, outcome, selected_features, 
    ...
    ----------
    test_data : DataFrame
        A pandas DataFrame containing a single test record, including the target outcome and necessary features for inference.
        The DataFrame must have the same feature set as the model's training data.

    outcome : str
        The name of the target column (dependent variable) representing the outcome to be predicted.
        This can be a binary label (e.g., success/failure, 1/0) or continuous values.

    selected_features : list of str
        A list of feature names used by the model for making predictions.
        These features should match the model's input expectations.

    model_info : bytes or model object
        A serialized model or model object containing the predictive model used for inference.
        This parameter supports two types of inputs:
    ...
    options : dict
        A dictionary specifying configuration settings for counterfactual inference.
        ...
        - `preprocessor` (object, optional): Preprocessing object used to transform input data during inference (required for custom models).
            The preprocessor should be a bundled class containing methods that internally transform and inverse transform the data as required.
            Regression:

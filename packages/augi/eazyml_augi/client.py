...
def ez_augi(mode, outcome, train_file_path, data_type_dict,
    ...
    Parameters :
        ...
        - **selected_features_list** (list):
            List of derived features on which model is trained.
    ...
    try:
        ...
        if isinstance(train_file_path, str):
            train_data, _ = utils.get_df(train_file_path, data_source=data_source)
        elif isinstance(train_file_path, pd.DataFrame):
            train_data = train_file_path
        else :
            return {
                "success": False,
                "message": 'train_file_path should be of either string or DataFrame'
                }
...
def ez_validate(mode, outcome, augi_insights, train_file_path, test_file_path,
    ...
    Parameters :
        ...
        - **augi_insights** (dict):
            Augmented Intelligence insights provided by ez_augi.
        - **train_file_path** (str):
    ...
    try:
        ...
        if not isinstance(augi_insights, dict):
            return {
                    "success": False,
                    "message": "Please provide augi insights which you got from ez_augi"
                    }
        if not isinstance(data_type_dict, dict):
            return {
                    "success": False,
        ...
        for col in set(augi_insights.keys()):
        ...
        insight_columns = [outcome, 'Augmented Intelligence Insights',
            'Insight Scores']
        if augi_insights['columns'] != insight_columns:
            return {
                    "success": False,
                    "message": "Please provide augi insights which you got from ez_augi"
                    }
        if ("data_source" in options and options[
            "data_source"] == "parquet"):
            data_source = "parquet"
        test_data, _ = utils.get_df(test_file_path, data_source=data_source)
        train_data, _ = utils.get_df(train_file_path, data_source=data_source)
        for col in data_type_dict.keys():
        ...
        if "record_number" in options and options["record_number"]:
            ...
            if is_list(record_number):
                ...
                record_number = [record_number]
            insights_count = len(augi_insights['data'])
        ...
        for sublist in augi_insights['data']:
            if len(sublist) != 3:
                return {
                        "success": False,
                        "message": "Please provide augi insights which you got from ez_augi"
        ...
        if not g.AUGI_VALIDATE_TRAIN:
            ...
            res = utils.calculate_validation_score(
                mode, test_data, outcome, augi_insights,
